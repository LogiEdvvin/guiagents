...
class PeftModelForPix2StructVision(PeftModel):

    def __init__(self, model: nn.Module, peft_config: PeftConfig, adapter_name: str = "default"):
    ...
    def forward(
        ...
        **kwargs
        ...
        if not peft_config.is_prompt_learning:
            with self._enable_peft_forward_hooks():
        ...
        return self.base_model(flattened_patches=flattened_patches,attention_mask=attention_mask)
...
class PolicyModel(nn.Module):
    def __init__(self, config):
        ...
        self._num_x_bins = 10
        self._num_y_bins = 10
        vision_model = Pix2StructVisionModel.from_pretrained(config['encoder_path'])
        lora_config = LoraConfig(peft_type=PeftModelForPix2StructVision, inference_mode=False, r=2, lora_alpha=32, lora_dropout=0.1, target_modules=['query', 'value'])
        self.vision_model = get_peft_model(vision_model, lora_config)
        self.lin_dim_reduce = nn.Linear(in_features=768, out_features=128)
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=128, nhead=4, batch_first=True),
            num_layers=1
        )
        self.decoder_tgt = nn.Parameter(pt.zeros(1, 128))
        self.linear = nn.Linear(in_features=128+(10+10+11+11)*4, out_features=128)
        self.lin_out_x = nn.Linear(in_features=128, out_features=self._num_dx_bins)
        self.lin_out_y = nn.Linear(in_features=128, out_features=self._num_dy_bins)
    ...
    def forward(self, flattened_patches, attention_mask, dxdy_lags, xy_lags):
        ...
        vision_hidden = self.lin_dim_reduce(vision_hidden)
...
def _grad_acc_collate(batch):
    return [[x.unsqueeze(dim=0) if isinstance(x, pt.Tensor) else (x,) for x in data_point] for data_point in batch]
...
def script():
    ...
    # Create dataloader
    train_dl = DataLoader(train_data, batch_size=32, shuffle=True, drop_last=True, collate_fn=_grad_acc_collate)
    val_dl = DataLoader(val_data, batch_size=32, shuffle=False, drop_last=False)
    ...
    for epoch in range(config['train']['epoch_num']):
        ...
        for batch_i, batch in tqdm(enumerate(train_dl)):
            model.train()
            for (
                vid_id,
                rel_frame,
                ...
                visual_inputs = processor(images=screens[:,0,:], header_text=caption, return_tensors="pt", is_vqa=True)
                ...
                x_pred, y_pred = model(
                    ...
                    dxdy_lags=mt_lags_dxdy,
                    xy_lags=mt_lags_xy,
                )

                loss_x = crit_x(x_pred, label[:,0])
                loss_y = crit_y(y_pred, label[:,1])
                loss = loss_x + loss_y
                loss.backward()
                loss_sum += loss.item()

                del flattened_patches
                del visual_attention_mask
                del mt_lags_dxdy
                del mt_lags_xy
                del label
                del loss
            ...
            if step % config['train']['logging_step'] == 0:
                loss_sum /= config['train']['logging_step']
